Veles Core version 0.13.0 is now available from:

  <https://veles.org/bin/veles-core-0.13.0/>
...
- Adding `dbcache=100` in veles.conf
...
For example:
    ...
    $ src/veles-cli -stdin walletpassphrase
    mysecretcode
    120
...
Various code modernizations have been done. The Veles Core code base has
...
HD wallets are incompatible with older versions of Veles Core.

[Pull request](https://github.com/velescore/veles/pull/8035/files), [BIP 32](https://github.com/bitcoin/bips/blob/master/bip-0032.mediawiki)
...
proposed parameters, a future release of Veles Core is expected that
...
The two phases are distinct in the Veles-Qt GUI. During the first one,
...
- Each ZMQ notification now contains an up-counting sequence number that allows
  ...
  PR [#7762](https://github.com/velescore/veles/pull/7762).
...
- #7550 `8b958ab` Input-from-stdin mode for veles-cli (laanwj)
...
- #7613 `3798e5d` Add autocomplete to veles-qt's console window (GamerSg)
...
- #7636 `fb0ac48` Add veles address label to request payment QR code (makevoid)
- #8231 `760a6c7` Fix a bug where the SplashScreen will not be hidden during startup (jonasschnelli)
- #8256 `af2421c` BUG: veles-qt crash (fsb4000)
- #8257 `ff03c50` Do not ask a UI question from velesd (sipa)
- #8288 `91abb77` Network-specific example address (laanwj)
- #7707 `a914968` UI support for abandoned transactions (jonasschnelli)
- #8207 `f7a403b` Add a link to the Veles-Core repository and website to the About Dialog (MarcoFalke)
...
- #8047 `37f9a1f` Test_framework: Set wait-timeout for velesd procs (MarcoFalke)
...
- #8164 `0f24eaf` [Veles-Tx] fix missing test fixtures, fix 32bit atoi issue (jonasschnelli)
...
- #7744 `d8e862a` test_framework: detect failure of velesd startup (laanwj)
...
- #7975 `fc23fee` Update veles-core GitHub links (MarcoFalke)
