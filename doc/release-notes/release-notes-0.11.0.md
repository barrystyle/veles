Veles Core version 0.11.0 is now available from:

  <https://veles.org/bin/veles-core-0.11.0/>
...
For example, add the following to `veles.conf`:
...
related to the blockchain in the veles system: the raw blocks as received over 
the network (blk???.dat), the undo data (rev???.dat), the block index and the 
UTXO set (both LevelDB databases). The databases are built from the raw data.

Block pruning allows Veles Core to delete the raw block and undo data once 
...
block index and UTXO databases. The minimum was chosen so that Veles Core will 
...
For an example script see [veles-submittx](https://github.com/laanwj/veles-submittx).
...
- #5334 `cf87536` libb*bitcoinconsensus: Add pkg-config support
...
- #6269 `95aca44` gitian: Use the new veles-detached-sigs git repo for OSX signatures
...
- #5729 `9c4a5a5` Allow unit changes for read-only VelesAmountField
- #5753 `0f44672` Add veles logo to about screen
...
- #5809 `46bfbe7` Add veles-cli man page
...
- #5849 `41259ca` contrib/init/velesd.openrc: Compatibility with previous OpenRC init script variables
...
- #6116 `30dc3c1` [OSX] rename Veles-Qt.app to Veles-Core.app
- #6168 `b3024f0` contrib/linearize: Support linearization of testnet blocks
- #6098 `7708fcd` Update Windows resource files (and add one for veles-tx)
