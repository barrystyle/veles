Veles Core version 0.18.0 is now available from:

  <https://veles.network/download.en.html>

This is a new major version release, including new features, various bug
fixes and performance improvements, as well as updated translations.

Veles Core was among the first blockchain projects that ported masternodes
to the latest Bitcoin 0.18 codebase, alongside with all Veles-specific functionality
such as multiple mining algorithms, dynamic block rewards and dynamic halving
system. 
...
Masternode
----------
- Fixes possibility of Masternode deadlock
- Fixes masternode NEW_START_REQUIRED status and caching mncache.dat
...
- The `whitelistforcerelay` option is used to relay transactions from
  ...
  [contacting](https://velescore.org/en/contact/) the Veles Core
...
- A new short [document](https://github.com/velescore/veles/blob/master/doc/JSON-RPC-interface.md)
  about the JSON-RPC interface describes cases where the results of an
  RPC might contain inconsistencies between data sourced from different
  subsystems, such as wallet state and mempool state.  A note is added
  to the [REST interface documentation](https://github.com/velescore/veles/blob/master/doc/REST-interface.md)
  indicating that the same rules apply.

- Further information is added to the [JSON-RPC
  documentation](https://github.com/velescore/veles/blob/master/doc/JSON-RPC-interface.md)
  about how to secure this interface.

- A new [document](https://github.com/velescore/veles/blob/master/doc/veles-conf.md)
  about the `veles.conf` file describes how to use it to configure
  Veles Core.

- A new document introduces Veles Core's BIP174 [Partially-Signed
  Veles Transactions
  (PSBT)](https://github.com/velescore/veles/blob/master/doc/psbt.md)
...
- The [output script
  descriptor](https://github.com/velescore/veles/blob/master/doc/descriptors.md)
...
- The minimum required version of Qt (when building the GUI) has been
  ...
  system](https://github.com/velescore/veles/blob/master/depends/README.md)
  provides 5.9.7)
...
This section describes planned changes to Veles Core that may affect
other Veles software and services.

- Since version 0.16.0, Veles Core’s built-in wallet has defaulted to
  ...
  widely-used software.  Starting with Veles Core 0.20 (expected about
  a year after 0.18), Veles Core will default to native segwit
  ...
  to bech32 addresses, and if the Veles Core project sees enough
  additional adoption, it will instead default to bech32 receiving
  addresses in Veles Core 0.19 (approximately November 2019).
  ...
  set the `addresstype=bech32` configuration option in any Veles Core
...
## Masternode
- #39 fix possible masternode deadlock, fixes #37 (uhliksk)
- #42 fix masternode ''new start required" status and caching mncache.dat (mdfkbtc, uhliksk)
...
- #12490 Remove deprecated wallet rpc features from `veles_server` (jnewbery)
- #14138 Set `encrypted_batch` to nullptr after delete. Avoid double free in the case of NDEBUG (practicalswift)
- #14168 Remove `ENABLE_WALLET` from `libveles_server.a` (jnewbery)
...
- #13926 [Tools] veles-wallet - a tool for creating and managing wallets offline (jnewbery)
...
- #14162 Also log and print messages or questions like velesd (MarcoFalke)
...
- #14521 qt, docs: Fix `veles-qt -version` output formatting (hebasto)
...
- #11625 Add VelesApplication & RPCConsole tests (ryanofsky)
...
- #14204 Move `interfaces/*` to `libveles_server` (laanwj)
...
- #15154 Configure: veles-tx doesn't need libevent, so don't pull it in (luke-jr)
...
- #15388 Makefile.am: add rule for src/veles-wallet (Sjors)
...
- #11526 Visual Studio build configuration for Veles Core (sipsorcery)
- #15110 build\_msvc: Fix the build problem in `libveles_server` (Mr-Leshiy)
...
- #13953 Fix deprecation in veles-util-test.py (isghe)
...
- #12775 Integration of property based testing into Veles Core (Christewart)
...
- #12246 Bugfix: Only run veles-tx tests when veles-tx is enabled (luke-jr)
...
- #14092 Dry run `bench_veles` as part `make check` to allow for quick identification of assertion/sanitizer failures in benchmarking code (practicalswift)
...
- #14985 Remove `thread_local` from `test_veles` (MarcoFalke)
...
- #14186 veles-cli: don't translate command line options (HashUnlimited)
- #14057 logging: Only log `using config file path_to_veles.conf` message on startup if conf file exists (leishman)
...
- #12255 Update veles.service to conform to init.md (dongcarl)
...
- #14497 Add doc/veles-conf.md (hebasto)
...
- #15354 Add missing `veles-wallet` tool manpages (MarcoFalke)
...
- #15765 correct velesconsensus_version in shared-libraries.md (fanquake)
- #15792 describe onlynet option in doc/tor.md (jonatack)
- #15802 mention creating application support veles folder on OSX (JimmyMow)
