Veles Core version 0.17.0 is now available from:

  <https://velescore.org/bin/veles-core-0.17.0/>
...
- `-includeconf=<file>` can be used to include additional configuration files.
  Only works inside the `veles.conf` file, not inside included files or from
  ...
  as veles.conf will still include `relative.conf`.
...
Previously, wallets could only be loaded or created at startup, by specifying `-wallet` parameters on the command line or in the veles.conf file. It is now possible to load, create and unload wallets dynamically at runtime:
...
prefixing the option with the network, such as:
    ...
    main.uacomment=veles
    test.uacomment=veles-testnet
...
be used in V0.17 by starting velesd with the '-deprecatedrpc=accounts'
...
BIP 174 Partially Signed Veles Transactions support
-----------------------------------------------------

[BIP 174 PSBT](https://github.com/bitcoin/bips/blob/master/bip-0174.mediawiki) is an interchange format for Veles transactions that are not fully signed
...
Overall, the construction of a fully signed Veles transaction goes through the
following steps:
...
- An **Extractor** produces a valid Veles transaction (in network format)
...
- The new RPC `getzmqnotifications` returns information about active ZMQ
  notifications.
- When veles is not started with any `-wallet=<path>` options, the name of
  the default wallet returned by `getwalletinfo` and `listwallets` RPCs is
  now the empty string `""` instead of `"wallet.dat"`. If veles is started
...
- `signrawtransaction` is deprecated and will be fully removed in v0.18. To use
  `signrawtransaction` in v0.17, restart velesd with
...
- When running velesd with `-debug` but without `-daemon`, logging to stdout
...
meaning the `-txindex` flag can be toggled without a full reindex. If velesd
...
- #11962 `0a01843` add seed.veles.sprovoost.nl to DNS seeds (Sjors)
...
- #12723 `310dc61` Qt5: Warning users about invalid-BIP21 URI veles:// (krab)
...
- #13528 `b877c39` Move VelesGUI initializers to class, fix initializer order warning (laanwj)
...
- #13486 `66e1a08` Move rpc/util.cpp from libbitcoin/bips-util to libbitcoin/bips-server (ken2812221)
...
- #13604 `75bea05` Add depends 32-bit arm support for veles-qt (TheCharlatan)
...
- #13696 `2ab7208` Add aarch64 qt depends support for cross compiling veles-qt (TheCharlatan)
...
- #11771 `2dbc4a4` Change invalidtxrequest to use VelesTestFramework (jnewbery)
...
- #12443 `07090c5` Move common args to veles.conf (MarcoFalke)
...
- #11772 `0630974` Change invalidblockrequest to use VelesTestFramework (jnewbery)
...
- #11773 `f0f9732` Change `feature_block.py` to use VelesTestFramework (jnewbery)
- #12839 `40f4baf` Remove travis checkout depth (laanwj)
- #11817 `2a09a78` Change `feature_csv_activation.py` to use VelesTestFramework (jnewbery)
...
- #12856 `27278df` Add Metaclass for VelesTestFramework (WillAyd)
...
- #12882 `d96bdd7` Make `test_veles` pass under ThreadSanitzer (clang). Fix lock-order-inversion (potential deadlock) (practicalswift)
...
- #13687 `9d26b69` travis: Check that ~/.veles is never created (MarcoFalke)
...
- #13811 `660abc1` travis: Run `bench_veles` once (MarcoFalke)
...
- #12102 `7fb8fb4` Apply hardening measures in velesd systemd service file (Flowdalic)
- #12652 `55f490a` veles-cli: Provide a better error message when velesd is not running (practicalswift)
...
- #12769 `35eb9d6` Add systemd service to velesd in debian package (ghost)
...
- #13374 `56f6936` utils and libraries: checking for veles address in translations (kaplanmaxe)
...
- #13603 `453ae5e` veles-tx: Stricter check for valid integers (domob1812)
...
- #13725 `07ce278` Fix veles-cli --version (Empact)
...
- #12877 `2b54155` Use velesd in Tor documentation (knoxcard)
...
- #13346 `60f0358` update veles-dot-org links in release-process.md (fanquake)
...
- #13742 `d2186b3` Adjust velescore.org links (MarcoFalke)
...
- #13905 `73a09b4` fixed veles-cli -help output for help2man (hebasto)
