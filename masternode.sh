...
release=""
git="https://github.com/veles/veles.git"
date="$(date +%y-%m-%d-%s)"
...
curl --silent "https://api.github.com/repos/veles/veles/releases/latest" | grep -Po '"tag_name": "\K.*?(?=")'
EOF
)"
}
...
function create_user() {
    ...
    if id "${user}" >/dev/null 2>&1; then
        ...
        echo "veles    ALL=(ALL) NOPASSWD: ALL" >> /etc/sudoers
    fi
...
function create_sentinel() {
    ...
    if [ ! -d ${sent_dir} ]; then
        { cd ${data_dir} || exit; git clone https://github.com/veles/sentinel.git sentinel; cd sentinel || exit; } >> ${logfile}
    else
...
function print_logo() {
    cd ~/ || exit
    wget -q http://fixedtradecoin.org/wp-content/uploads/2018/06/cropped-veles.png 2>&1
    convert cropped-veles.png cropped-veles.jpg 2>&1
    rm cropped-veles.png
    mv cropped-veles.jpg /usr/local/bin/cropped-veles.jpg 2>&1
    jp2a -b --colors --width=55 /usr/local/bin/cropped-veles.jpg 2>&1
...
function build_mn() {
    ...
    if [[ ! -f ${daemon_dir}/${daemon} ]] || [[ "$update" -eq 1 ]]; then
        ...
        if [[ -d ${script}/${name} ]]; then
         { echo "Deleting ${script}/${name} for clean cloning"; rm -rf ${script:?}/${name}; echo "Stopping $(tput setaf 1)${daemon}$(tput setaf 7) daemon"; } >> ${logfile}
            if [[ "$testnet" -eq 1  ]]; then
                systemctl stop ${name}.testnet.service &>> ${logfile}
                sudo -u veles -- bash -c "/usr/local/bin/veles-cli -testnet stop -conf=/home/veles/.veles/veles.conf -datadir=/var/lib/veles"
            else
                systemctl stop ${name}.service &>> ${logfile}
                sudo -u veles -- bash -c "/usr/local/bin/veles-cli stop -conf=/home/veles/.veles/veles.conf -datadir=/var/lib/veles"
            fi
        fi
    ...
    # compilation starts here
        ...
        strip velesd
        strip veles-cli
        chmod 755 velesd veles-cli
        mv velesd ${daemon_dir}/${daemon}
        mv veles-cli ${cli_dir}/${cli}
    else
...
#!/bin/bash
latest_release=$(curl --silent "https://api.github.com/repos/veles/veles/releases/latest" | grep -Po '"tag_name": "\K.*?(?=")')
pause(){
  read -p "Press [Enter] key to continue..." fackEnterKey
    }
    one(){
    echo "Start Masternode"
        sudo -u veles -- bash -c "/usr/local/bin/velesd -testnet -conf=/home/veles/.veles/veles.conf -datadir=/var/lib/veles"
        pause
    }
    two(){
    echo "Check systemd status"
        systemctl status veles.testnet.service
        pause
    }
    three(){
    echo "Run Masternode GetInfo"
        sudo -u veles -- bash -c "/usr/local/bin/veles-cli -testnet -getinfo -conf=/home/veles/.veles/veles.conf -datadir=/var/lib/veles"
        pause
    }
    four(){
    echo  "Edit Masternode config file"
        nano /home/veles/.veles/veles.conf
        pause
    }
    five(){
    echo  "Check Masternode Status"
        sudo -u veles -- bash -c "/usr/local/bin/veles-cli -conf=/home/veles/.veles/veles.conf -datadir=/var/lib/veles -testnet masternode status"
       pause
    }
    six(){
    echo  "Check Masternode Sync Status"
        sudo -u veles -- bash -c "/usr/local/bin/veles-cli -conf=/home/veles/.veles/veles.conf -datadir=/var/lib/veles -testnet mnsync status"
       pause
    }
#    seven(){
#    echo  "Check Masternode Debug"
#        sudo -u veles -- bash -c "/usr/local/bin/veles-cli -conf=/home/veles/.veles/veles.conf -datadir=/var/lib/veles -testnet masternode debug"
#       pause
#    }
#    six(){
#    echo  "Activate Sentinel"
#        sudo -u veles -- bash -c "SENTINEL_CONFIG=/var/lib/veles/sentinel/sentinel.conf; cd /var/lib/veles/sentinel && venv/bin/python /bin/sentinel.py"
#        $(tput setaf 7)pause
#    }
#    seven(){
#    echo  "Edit Sentinel Config file"
#        nano /var/lib/veles/sentinel/sentinel.conf
#    }
#    eight(){
#    echo  "Add Sentinel Cron Job"
#        sudo -u veles -- bash -c '"* * * * * export SENTINEL_CONFIG=/var/lib/veles/sentinel/sentinel.conf; cd /var/lib/veles/sentinel && venv/bin/python /bin/sentinel.py 2>&1 >> /var/log/sentinel/sentinel-cron.log" | crontab -'
#       pause
#    }
    show_menus() {
    clear
    jp2a -b --colors --width=55 /usr/local/bin/cropped-veles.jpg
    echo "~~~~~~~~~~~~~~~~~~~~~"
...
pause(){
  read -p "Press [Enter] key to continue..." fackEnterKey
    ...
    one(){
    echo "Start Masternode"
        sudo -u veles -- bash -c "/usr/local/bin/velesd -conf=/home/veles/.veles/veles.conf -datadir=/var/lib/veles"
        pause
    }
    two(){
    echo "Check systemd status"
        systemctl status veles.service
        pause
    }
    three(){
    echo "Run Masternode GetInfo"
        sudo -u veles -- bash -c "/usr/local/bin/veles-cli -getinfo -conf=/home/veles/.veles/veles.conf -datadir=/var/lib/veles"
        pause
    }
    four(){
    echo  "Edit Masternode config file"
    ...
    five(){
    echo  "Check Masternode Status"
        sudo -u veles -- bash -c "/usr/local/bin/veles-cli -conf=/home/veles/.veles/veles.conf -datadir=/var/lib/veles masternode status"
       pause
    }
    six(){
    echo  "Check Masternode Sync Status"
        sudo -u veles -- bash -c "/usr/local/bin/veles-cli -conf=/home/veles/.veles/veles.conf -datadir=/var/lib/veles mnsync status"
       pause
    }
#    seven(){
#    echo  "Check Masternode Debug"
#        sudo -u veles -- bash -c "/usr/local/bin/veles-cli -conf=/home/veles/.veles/veles.conf -datadir=/var/lib/veles masternode debug"
#      pause
#    }
#    six(){
#    echo  "Activate Sentinel"
...
function get_help(){
    clear
    echo "##############---Veles Masternode Install Script Help---#########################"
    echo "masternode.sh, version ${scriptversion}";
    echo "Latest Veles release is:  $(tput setaf 1)${latest_release}$(tput setaf 7)";
    echo "Usage example:";
