...
void OverviewPage::setBalance(const interfaces::WalletBalances& balances)
{
    ...
    if (walletModel->privateKeysDisabled()) {
        ...
        ui->labelTotalText->setText(BitcoinUnits::formatWithUnit(unit, balances.watch_only_balance + balances.unconfirmed_watch_only_balance + balances.immature_watch_only_balance, false, BitcoinUnits::separatorAlways));
    } else {
...
void OverviewPage::updatePrivateSendProgress()
{
    ...
    if(nMaxToAnonymize >= privateSendClient.nPrivateSendAmount * COIN) {
        ...
        strAmountAndRounds = "<span>" + QString(BitcoinUnits::factor(nDisplayUnit) == 1 ? "" : "~") + strMaxToAnonymize +
...
void OverviewPage::togglePrivateSend()
{
    ...
    if (!privateSendClient.fEnablePrivateSend) {
        ...
        if(pwallet->GetBalance() < nMinAmount){
...
void OverviewPage::DisablePrivateSendCompletely() {
    ...
    if (nWalletBackups <= 0) {
        ui->labelPrivateSendEnabled->setText("<span>(" + tr("Disabled") + ")</span>");
    }
